...
def shelf_y(name, cy, x_wall, sign, n_boxes=3, c_shelf=C_SHELF, c_box=C_BOX):
    """Create a shelf along Y against a wall parallel to the Y axis."""
...
def shelf_x(name, cx, y_wall, sign, n_boxes=3, c_shelf=C_SHELF, c_box=C_BOX):
    """Create a shelf along X against a wall parallel to the X axis."""
...
def desk(name, cx, cy, facing='x', c_desk=C_DESK):
    """Create a desk with four legs and a table top."""
...
def chair(name, cx, cy, back_dir='+y', c_chair=C_CHAIR):
    """Create a chair with a backrest on the requested side."""
    ...
    links.append(solid_link(
        f'{name}_seat',
        (cx, cy, SEAT_H - CHAIR_T / 2),
        (SEAT_W, SEAT_W, CHAIR_T),
        c_chair,
    ))
    # backrest
    bx = cx + dx * off
    ...
    for i, (sx, sy) in enumerate([(-1, -1), (-1, 1), (1, -1), (1, 1)]):
        links.append(solid_link(
            f'{name}_leg{i}',
            (cx + sx * lo, cy + sy * lo, leg_h / 2),
            (CHAIR_LEG, CHAIR_LEG, leg_h),
            c_chair,
        ))
    return links
...
def plant(name, cx, cy, c_pot=C_POT, c_plant=C_PLANT):
    """Create a decorative potted plant."""
    links = []
    links.append(solid_link(
        f'{name}_pot',
        (cx, cy, POT_H / 2),
    ...
    links.append(solid_link(
        f'{name}_leaves',
        (cx, cy, POT_H + LEAF_H / 2),
        (LEAF_W, LEAF_W, LEAF_H),
        c_plant,
    ))
    return links
